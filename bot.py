...
TEXTS = {
    'uz': {
        ...
        'cleaning_day': "🧹 Tozalash kuni - bron qilish mumkin emas",
...
Kutubxonachi yoki administratorga murojaat qiling.""",
    ...
    'ru': {
        ...
        'room_free': "Свободно",
        'room_occupied': "🟡 {start_time}-{end_time} - {group_name}",
        'cleaning_day': "🧹 День уборки - бронирование недоступно",
...
• Последняя пятница месяца - день уборки
...
Обратитесь к библиотекарю или администратору.""",
    ...
    'en': {
        ...
        'room_free': "Free",
        'room_occupied': "🟡 {start_time}-{end_time} - {group_name}",
        'cleaning_day': "🧹 Cleaning day - booking unavailable",
...
• Last Friday of month is cleaning day
...
def is_last_friday_of_month(date):
    ...
    if date.weekday() != 4:  # Если не пятница
        return False
    ...
    # Находим последнюю пятницу месяца
    days_until_friday = (4 - last_date.weekday()) % 7
    if days_until_friday == 0 and last_date.weekday() == 4:
        last_friday = last_date
    else:
        last_friday = last_date - datetime.timedelta(days=(last_date.weekday() - 4) % 7)
...
def generate_qr_code(booking_id, user_id, room_id, date, time_slot, group_name, lang='uz'):
    ...
    captions = {
        'uz': "Universitet Axborot resurs markazi\nMuhokama xonalarini bron qilish",
...
def get_dates_keyboard(user_id):
    """Клавиатура для выбора даты (без субботы и воскресенья, исключая последнюю пятницу месяца)"""
    keyboard = []
    ...
    while days_added < BOOKING_ADVANCE_DAYS:
        ...
        # Пропускаем последнюю пятницу месяца (cleaning day)
        if is_last_friday_of_month(date):
            continue
...
async def go_to_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Если сообщение нельзя редактировать (фото, нет текста и т.д.)
...
async def today_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, не является ли сегодня последней пятницей месяца
    if is_last_friday_of_month(today):
        await query.edit_message_text(
            text=t(user_id, 'cleaning_day'),
            reply_markup=get_back_to_menu_keyboard(user_id)
        )
        return SELECTING_ACTION
    ...
    for room_id, room_name in ROOMS.items():
        # Экранируем название комнаты для Markdown
        safe_room_name = escape_markdown(room_name)
        text += f"*{safe_room_name}*\n"

        occupied = db.get_available_slots(room_id, today_str)

        if not occupied:
            free_text = escape_markdown(t(user_id, 'room_free'))
            text += f"   {free_text}\n"
        else:
            for booking in occupied:
                group_name = escape_markdown(booking['group_name'])
                start = escape_markdown(booking['start_time'])
                end = escape_markdown(booking['end_time'])
                text += f"   🟡 {start}\\-{end} \\- {group_name}\n"
    ...
    try:
        ...
        logger.error(f"Error sending schedule with MarkdownV2: {e}")
        # Fallback без форматирования
        text_plain = t(user_id, 'today_schedule')
        for room_id, room_name in ROOMS.items():
            text_plain += f"{room_name}\n"
            occupied = db.get_available_slots(room_id, today_str)
            if not occupied:
                text_plain += f"   {t(user_id, 'room_free')}\n"
            else:
                for booking in occupied:
                    text_plain += f"   🟡 {booking['start_time']}-{booking['end_time']} - {booking['group_name']}\n"
            text_plain += "\n"
